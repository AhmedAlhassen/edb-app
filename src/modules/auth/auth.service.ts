...
export class AuthService {
  ...
  async isAut(username) {
    this.logger.log(
      'LoginUseCases execute',
      `The user ${username} have been logged.`,
    );
    const user = await this.userRepo.getUserByUsername(username);
    const { password, hach_refresh_token, ...info } = user;
    return info;
  }
  ...
  async findAllPermissionsOfUser(username: string) {
    const perm = await this.userRepo.findAllPermissionsOfUser(username);
    return await this.userRepo.findAllPermissionsOfUser(username);
  }
}
